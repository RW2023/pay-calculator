    ...
    return (
        <section ref={ref} className="w-full mt-4 space-y-6">
            ...
            <div className="card shadow rounded-xl p-6 border-t-4 border-neutral-dark bg-[var(--background)] text-[var(--foreground)] transition-colors duration-300">
                ...
                <ul className="mt-1 text-base space-y-1">
                    {deductionRows.map(({ label, value, colorClass }) => (
                        <li key={label}>
                            {label}:{' '}
                            <span className={`font-bold ${colorClass}`}>${value.toFixed(2)}</span>
                        </li>
                    ))}
                </ul>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-6">
                {/* Net Pay card uses teal in light, neutral in dark */}
                <div
                    className="card shadow-xl rounded-xl p-6 flex flex-col items-center border-none bg-[var(--color-teal)] text-[var(--background)] dark:bg-[var(--color-neutral-dark)] dark:text-[var(--foreground)] transition-colors duration-300"
                >
